"""
Registry Task Management Endpoints
- Register CodeCollections, index repositories, parse codebundles
- Enhance metadata with AI, generate system metrics
"""
...
router = APIRouter(prefix="/api/v1/tasks", tags=["registry-tasks"])
...
async def trigger_sync_single_collection(
    ...
    try:
        from app.tasks.registry_tasks import sync_single_collection_task
...
async def trigger_parse_collection_codebundles(
    ...
    try:
        from app.tasks.registry_tasks import parse_collection_codebundles_task
...
async def trigger_enhance_codebundles(
    ...
    try:
        from app.tasks.registry_tasks import enhance_all_codebundles_task
...
async def trigger_enhance_single_codebundle(
    ...
    try:
        from app.tasks.registry_tasks import enhance_single_codebundle_task
...
async def trigger_generate_metrics(
    ...
    try:
        from app.tasks.registry_tasks import generate_metrics_task
...
async def health_check():
    """Health check for registry task system"""
    try:

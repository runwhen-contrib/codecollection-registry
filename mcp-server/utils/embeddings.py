...
class EmbeddingGenerator:
    ...
    def _init_azure(self):
        ...
        Supports separate embedding configuration via AZURE_OPENAI_EMBEDDING_* vars.
        Falls back to main AZURE_OPENAI_* vars if embedding-specific vars not set.
        """
        try:
            from openai import AzureOpenAI
            
            # Embedding-specific settings with fallback to main Azure OpenAI settings
            ...
            api_version = os.getenv("AZURE_OPENAI_EMBEDDING_API_VERSION") or os.getenv("AZURE_OPENAI_API_VERSION", "2024-02-15-preview")
            ...
            if endpoint and api_key:
                ...
                # Log which endpoint is being used for clarity
                is_separate = bool(os.getenv("AZURE_OPENAI_EMBEDDING_ENDPOINT"))
                endpoint_type = "dedicated embedding" if is_separate else "main"
                logger.info(f"Azure OpenAI embedding client initialized ({endpoint_type} endpoint: {endpoint})")
            else:
...
def get_embedding_generator(prefer_local: bool = False) -> EmbeddingGenerator:
    ...
    Checks for embedding-specific Azure credentials first (AZURE_OPENAI_EMBEDDING_*),
    then falls back to main Azure credentials (AZURE_OPENAI_*).
    ...
    if prefer_local:
        ...
        # Check if Azure credentials are available (embedding-specific or main)
